...
pub fn getModule(b: *std.Build, comptime prefix_path: []const u8) *Build.Module {
    return b.createModule(.{
        .root_source_file = .{ .path = prefix_path ++ "renderkit/renderkit.zig" },
    });
}

/// prefix_path is the path to the gfx build.zig file relative to your build.zig.
/// prefix_path is used to add package paths. It should be the the same path used to include this build file.
pub fn addRenderKitToArtifact(b: *Build, exe: *Build.Step.Compile, target: Build.ResolvedTarget, comptime prefix_path: []const u8) void {
    _ = b;
    if (prefix_path.len > 0 and !std.mem.endsWith(u8, prefix_path, "/")) @panic("prefix-path must end with '/' if it is not empty");
    addOpenGlToArtifact(exe, target);
}

fn addOpenGlToArtifact(artifact: *Build.Step.Compile, target: Build.ResolvedTarget) void {
